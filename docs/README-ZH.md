...
| 完整 compute kernels | 下游项目：`zarrow-compute` |
| Parquet | 不在本仓库范围内 |
...
- 面向下游 `zarrow-compute` 的 compute 框架基础
...
| `zarrow-core` | 核心能力接口，适合不需要 IPC 或 FFI 依赖的下游库（如 `zarrow-compute`）。 |
...
`zarrow` 提供面向下游（如 `zarrow-compute`）的 compute 框架基础。
...
`zarrow` 在本仓库中**不会**直接承接完整 compute 函数目录。算术、比较、filter、sort、cast、aggregate 等具体 kernel 应在下游库（如 `zarrow-compute`）中实现。
...
- `zarrow-compute`：基于 `zarrow` / `zarrow-core` 的 Arrow compute kernels。
...
- 以稳定 compute 框架边界支撑 `zarrow-compute`；
- 增加真实互操作 demo；
- 持续完善所有权、内存布局、生命周期文档。
